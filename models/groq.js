...
// Simulated data structure for slots
const slotData = {
...
function parseDate(inputDate) {
    // Try to parse input using date-fns
    const parsedDate = parse(inputDate, "d MMMM", new Date());
    if (isValid(parsedDate)) {
        return format(parsedDate, "d MMMM"); // Return as "27 November"
    }
    return null;
}
...
function book_slot(date, time) {
    ...
    if (slotIndex > -1) {
        availableSlots.splice(slotIndex, 1); // Remove the booked slot
...
async function getGroqChat(text, stack) {
    ...
    try {
        if (/available slots/i.test(text)) {
            ...
            const dateMatch = text.match(/on (\d{1,2} \w+)/i); // Match date like "27 November"
            ...
            if (dateMatch && timeMatch) {
                ...
                if (!date) {
                    responseContent = "The specified date is invalid. Please use formats like '27 November'.";
                } else {
                    responseContent = book_slot(date, time);
                }
            } else {
